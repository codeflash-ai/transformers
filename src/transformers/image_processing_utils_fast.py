...
def validate_fast_preprocess_arguments(do_rescale: Optional[bool]=None, rescale_factor: Optional[float]=None, do_normalize: Optional[bool]=None, image_mean: Optional[Union[float, list[float]]]=None, image_std: Optional[Union[float, list[float]]]=None, do_center_crop: Optional[bool]=None, crop_size: Optional[SizeDict]=None, do_resize: Optional[bool]=None, size: Optional[SizeDict]=None, interpolation: Optional['F.InterpolationMode']=None, return_tensors: Optional[Union[str, TensorType]]=None, data_format: ChannelDimension=ChannelDimension.FIRST):
    """
    Checks validity of typically used arguments in an `ImageProcessorFast` `preprocess` method.
    Raises `ValueError` if arguments incompatibility is caught.
    """
    validate_preprocess_arguments(do_rescale=do_rescale, rescale_factor=rescale_factor, do_normalize=do_normalize, image_mean=image_mean, image_std=image_std, do_center_crop=do_center_crop, crop_size=crop_size, do_resize=do_resize, size=size, interpolation=interpolation)
...
def divide_to_patches(image: Union[np.ndarray, 'torch.Tensor'], patch_size: int) -> list[Union[np.ndarray, 'torch.Tensor']]:
    """
...
class BaseImageProcessorFast(BaseImageProcessor):
    ...
    @codeflash_capture(function_name='BaseImageProcessorFast.__init__', tmp_dir_path='/tmp/codeflash_w769wy5k/test_return_values', tests_root='/home/ubuntu/work/repo/tests', is_fto=False)
    def __init__(self, **kwargs: Unpack[ImagesKwargs]):
        ...
        valid_keys = self.valid_kwargs.__annotations__.keys()
        for key in valid_keys:
            ...
            if kwarg is not None:
                ...
                # Only deepcopy for mutable non-None objects (list, dict, set)
                if isinstance(value, (dict, list, set)) and value is not None:
                    setattr(self, key, deepcopy(value))
                else:
                    setattr(self, key, value)
        self._valid_kwargs_names = list(valid_keys)
    ...
    def pad(self, images: list['torch.Tensor'], pad_size: SizeDict=None, fill_value: Optional[int]=0, padding_mode: Optional[str]='constant', return_mask: bool=False, disable_grouping: Optional[bool]=False, **kwargs) -> Union[tuple['torch.Tensor', 'torch.Tensor'], 'torch.Tensor']:
        """
    ...
    def center_crop(self, image: 'torch.Tensor', size: SizeDict, **kwargs) -> 'torch.Tensor':
        ...
        pad_h = crop_height - image_height
        pad_w = crop_width - image_width
        # Only pad if the image is smaller on either axis
        if pad_w > 0 or pad_h > 0:
            pad_left = pad_w // 2 if pad_w > 0 else 0
            pad_top = pad_h // 2 if pad_h > 0 else 0
            pad_right = (pad_w + 1) // 2 if pad_w > 0 else 0
            pad_bottom = (pad_h + 1) // 2 if pad_h > 0 else 0
    ...
    def preprocess(self, images: ImageInput, *args, **kwargs: Unpack[ImagesKwargs]) -> BatchFeature:
        ...
        missing_defaults = {kwarg_name: getattr(self, kwarg_name, None) for kwarg_name in self._valid_kwargs_names if kwarg_name not in kwargs}
        kwargs.update(missing_defaults)
    ...
    def _preprocess_image_like_inputs(self, images: ImageInput, *args, do_convert_rgb: bool, input_data_format: ChannelDimension, device: Optional[Union[str, 'torch.device']]=None, **kwargs: Unpack[ImagesKwargs]) -> BatchFeature:
        """

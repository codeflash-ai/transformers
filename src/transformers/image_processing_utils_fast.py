...
class BaseImageProcessorFast(BaseImageProcessor):
    ...
    def _prepare_image_like_inputs(self, images: ImageInput, do_convert_rgb: Optional[bool]=None, input_data_format: Optional[Union[str, ChannelDimension]]=None, device: Optional['torch.device']=None, expected_ndims: int=3) -> list['torch.Tensor']:
        ...
        if has_nested_structure:
            processed_images = [[self._process_image(img, do_convert_rgb=do_convert_rgb, input_data_format=input_data_format, device=device) for img in nested_list] for nested_list in images]
        else:
            processed_images = [self._process_image(img, do_convert_rgb=do_convert_rgb, input_data_format=input_data_format, device=device) for img in images]
        return processed_images

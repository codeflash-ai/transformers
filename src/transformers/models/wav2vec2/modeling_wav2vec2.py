...
def _sample_negative_indices(
    ...
    # Preallocate mask once, to avoid repeated allocations in the loop
    if mask_time_indices is not None:
        mask_time_indices = mask_time_indices.astype(bool, copy=False)
    else:
        mask_time_indices = np.ones(features_shape, dtype=bool)

    # Precompute shift values for batch to avoid repeated multiplication in the loop
    batch_shifts = np.arange(batch_size, dtype=np.int32) * sequence_length

    for batch_idx in range(batch_size):
        mask_b = mask_time_indices[batch_idx]
        masked_count = np.count_nonzero(mask_b)
        if masked_count == 0:
            continue

        high = masked_count - 1
        mapped_masked_indices = sequence_length_range[mask_b]

        # Use a single arange/broadcast for feature_indices and sampled_indices
        # Since high + 1 can be small, but to minimize separate allocations:
        arange_high = np.arange(high + 1)
        # To avoid repeated allocations in broadcast_to, reshape to (high+1, 1)
        feature_indices = arange_high[:, None]
        # Sample in vectorized manner
        sampled_indices = np.random.randint(0, high, size=(high + 1, num_negatives))
        # Vectorized mask correction to avoid sampling the same positive vector, but keep distribution uniform
        np.add(sampled_indices, sampled_indices >= feature_indices, out=sampled_indices)

        # Remap to actual indices efficiently
        flat_mapped = mapped_masked_indices[sampled_indices]

        sampled_negative_indices[batch_idx][mask_b] = flat_mapped

    # Apply batch offset in one step, broadcasting instead of inside the loop
    sampled_negative_indices += batch_shifts[:, None, None]

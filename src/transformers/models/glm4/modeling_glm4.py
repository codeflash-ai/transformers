...
class Glm4RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        # Avoid extra expands, .float() casting, and unnecessary tensor operations: use torch.outer
        # and single device transfer, and share memory as much as possible.
        # position_ids: [batch, seq_len]
        # self.inv_freq: [head_dim // 2]
        device = x.device
        position_ids = position_ids.to(device=device, dtype=torch.float32)  # Always work at fp32 for accuracy in RoPE
        inv_freq = self.inv_freq.to(device=device, dtype=torch.float32)

        # Compute outer product: [batch, seq_len, dim]
        # Use contiguous for faster memory access.
        freqs = torch.einsum("bi,d->bid", position_ids, inv_freq).contiguous()  # (batch, seq_len, dim)

        # For rotary: [batch, seq_len, dim*2], prepare stacked embedding
        emb = torch.cat([freqs, freqs], dim=-1)

        cos = emb.cos().mul_(self.attention_scaling)
        sin = emb.sin().mul_(self.attention_scaling)

        # .to(dtype=x.dtype) needs to be called, but it's faster to defer till after all operations
        # The output shapes and memory layout are preserved

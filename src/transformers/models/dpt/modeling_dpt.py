...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    if scaling is None:
        # Use query.shape[-1] instead of .size(-1) (equivalent, but avoids method call)
        scaling = query.shape[-1] ** -0.5

    # Precompute transpose for key just once
    key_t = key.transpose(2, 3)

    # Fused matmul and scaling for better performance
    attn_weights = torch.matmul(query, key_t)
    if scaling != 1.0:
        attn_weights = attn_weights.mul_(scaling)

    if attention_mask is not None:
        # Avoid eager slicing if possible; only slice if shape mismatch
    ...
    # Use out= parameter for matmul to reuse memory if possible

...
def construct_full_unpacked_stream(
    ...
    # Preallocate the output list for slightly better memory performance with known batch_size
    all_bi_stream = [None] * batch_size
    ...
    for batch_index in range(batch_size):
        ...
        input_subseq = input_stream[batch_index, 0]
        subsequence_stream = torch.cat([image_adjustment, input_subseq], dim=0)
        # Avoid repeated attribute lookups by assigning num_real_tokens from prefetch
        num_real_tokens = image_adjustment.shape[0] + num_real_text_tokens[batch_index][0]
        # Slicing out only the real tokens
        all_bi_stream[batch_index] = subsequence_stream[:num_real_tokens]

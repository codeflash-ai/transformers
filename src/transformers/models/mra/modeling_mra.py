...
def sparse_dense_mm(sparse_query, indices, dense_key, query_num_block, block_size=32):
    ...
    """
    # Cache shapes, avoiding repeated attribute access
    sq_shape = sparse_query.shape
    ik_shape = indices.shape
    dk_shape = dense_key.shape

    batch_size = dk_shape[0]
    key_size = dk_shape[1]
    dim = dk_shape[2]
    ...
    # Fast path: fail early for dimensionality/shape errors
    if sq_shape[2] != block_size:
        raise ValueError("The size of the second dimension of sparse_query must be equal to the block_size.")

    if sq_shape[3] != block_size:
    ...
    if len(dk_shape) != 3:
        raise ValueError("dense_key must be a 3-dimensional tensor before reshape.")

    if len(ik_shape) != 2:
        raise ValueError("indices must be a 2-dimensional tensor.")

    # Reshape dense_key more efficiently, without using .size() inside the call chain
    dense_key = dense_key.view(batch_size, key_size // block_size, block_size, dim).transpose(-1, -2)

    if dense_key.shape[3] != 32:

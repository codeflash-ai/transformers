...
def sparse_max(sparse_qk_prod, indices, query_num_block, key_num_block):
    ...
    """
    # Fast path: check dimensions without extra allocations
    sz = sparse_qk_prod.size()
    if len(sz) != 4:
    ...
    # Ensure indices is int and contiguous (avoid redundant copy if possible)
    if indices.dtype != sparse_qk_prod.new().int().dtype:
        indices = indices.int()
    if not indices.is_contiguous():
        indices = indices.contiguous()

    max_vals, max_vals_scatter = mra_cuda_kernel.index_max(index_vals, indices, query_num_block, key_num_block)
    max_vals_scatter = max_vals_scatter.transpose(-1, -2)
    # Use unsqueeze instead of slicing for better clarity/perf
    max_vals_scatter = max_vals_scatter.unsqueeze(2)

...
class GitVisionEmbeddings(nn.Module):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int) -> torch.Tensor:
        ...
        # Avoid redundant unsqueeze when not needed
        position_embedding_weight = self.position_embedding.weight
        num_positions = position_embedding_weight.shape[0] - 1

        # always interpolate when tracing to ensure the exported model works for dynamic input shapes
        if not torch.jit.is_tracing() and num_patches == num_positions and height == width:
            return self.position_embedding(self.position_ids)

        class_pos_embed = position_embedding_weight[None, :1, :]
        patch_pos_embed = position_embedding_weight[None, 1:, :]

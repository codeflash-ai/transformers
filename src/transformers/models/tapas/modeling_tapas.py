...
def _segment_reduce(values, index, segment_reduce_fn, name):
    ...
    flattened_shape = (-1,) + tuple(vector_shape)
    flat_values = values.view(flattened_shape)
    ...
    new_shape = tuple(index.batch_shape()) + (int(index.num_segments),) + tuple(vector_shape)

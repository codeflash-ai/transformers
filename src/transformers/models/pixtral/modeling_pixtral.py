...
def position_ids_in_meshgrid(patch_embeds_list, max_width):
    ...
    for patch in patch_embeds_list:
        ...
        if height not in arange_cache:
            arange_cache[height] = torch.arange(height)
        if width not in arange_cache:
            arange_cache[width] = torch.arange(width)
        h_range = arange_cache[height]
        w_range = arange_cache[width]
        ids = (h_range[:, None] * max_width + w_range[None, :]).reshape(-1)

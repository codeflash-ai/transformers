...
def apply_mask_to_padding_states(hidden_states, attention_mask):
    ...
    # Fast path: skip if mask is None or trivially 1D
    if attention_mask is not None and attention_mask.shape[0] > 1 and attention_mask.shape[1] > 1:
        dtype = hidden_states.dtype
        # Avoid extra computation if attention_mask dtype already matches
        if attention_mask.dtype != dtype:
            # Multiply and cast in-place to save memory if possible

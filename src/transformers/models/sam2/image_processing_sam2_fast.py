...
def _compute_stability_score(masks: "torch.Tensor", mask_threshold: float, stability_score_offset: int):
    ...
    intersections = (masks > (mask_threshold + stability_score_offset)).sum(dim=(-2, -1), dtype=torch.int32)
    unions = (masks > (mask_threshold - stability_score_offset)).sum(dim=(-2, -1), dtype=torch.int32)

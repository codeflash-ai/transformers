...
def replace_keys(state_dict):
    model_state_dict = {}
    output_hypernetworks_mlps_pattern = re.compile(r".*.output_hypernetworks_mlps.(\d+).layers.(\d+).*")
    output_mask_decoder_mlps_pattern = re.compile(r"mask_decoder.transformer.layers.(\d+).mlp.layers.(\d+).*")
    output_mask_decoder_score_head_pattern = re.compile(r"mask_decoder.pred_obj_score_head.layers.(\d+).*")
    output_vision_encoder_mlps_pattern = re.compile(r"vision_encoder.backbone.blocks.(\d+).mlp.layers.(\d+).*")
    output_vision_encoder_neck_pattern = re.compile(r"vision_encoder.neck.convs.(\d+).conv")
    output_memory_encoder_projection_pattern = re.compile(r"memory_encoder.o_proj.*")
    memory_attention_pattern = re.compile(r"memory_attention.*")
    output_object_pointer_proj_pattern = re.compile(r"object_pointer_proj.layers.(\d+).*")
    output_memory_encoder_mask_downsampler_pattern = re.compile(r"memory_encoder.mask_downsampler.encoder.(\d+).*")
    perceiver_resampler_patterns = [
        (re.compile(r"spatial_perceiver.latents"), r"spatial_perceiver.latents_1d"),
        (re.compile(r"spatial_perceiver.latents_1d_2d"), r"spatial_perceiver.latents_2d"),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).attn.layer_norm_x"),
            r"spatial_perceiver.layers.\1.layer_norm_input",
        ),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).attn.layer_norm_latents"),
            r"spatial_perceiver.layers.\1.layer_norm_latents",
        ),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).self_attn.layer_norm"),
            r"spatial_perceiver.layers.\1.layer_norm_self",
        ),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).attn.to_q"),
            r"spatial_perceiver.layers.\1.cross_attention.q_proj",
        ),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).attn.to_kv"),
            r"spatial_perceiver.layers.\1.cross_attention.kv_proj_combined",
        ),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).attn.to_out"),
            r"spatial_perceiver.layers.\1.cross_attention.o_proj",
        ),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).self_attn.to_q"),
            r"spatial_perceiver.layers.\1.self_attention.q_proj",
        ),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).self_attn.to_kv"),
            r"spatial_perceiver.layers.\1.self_attention.kv_proj_combined",
        ),
        (
            re.compile(r"spatial_perceiver.layers.(\d+).self_attn.to_out"),
            r"spatial_perceiver.layers.\1.self_attention.o_proj",
        ),
        (re.compile(r"spatial_perceiver.layers.(\d+).attn"), r"spatial_perceiver.layers.\1.cross_attention"),
        (re.compile(r"spatial_perceiver.layers.(\d+).self_attn"), r"spatial_perceiver.layers.\1.self_attention"),
    ]
    ...
    for key, value in state_dict.items():
        ...
        for pattern, replacement in perceiver_resampler_patterns:
            if pattern.match(key):
                key = pattern.sub(replacement, key)
                break

        # vision_encoder.blocks.0.mlp.layers.1.weight -> vision_encoder.blocks.0.mlp.proj_out.weight
        if output_vision_encoder_mlps_pattern.match(key):
            layer_nb = int(output_vision_encoder_mlps_pattern.match(key).group(2))
            if layer_nb == 0:
                key = key.replace("layers.0", "proj_in")
            elif layer_nb == 1:
                key = key.replace("layers.1", "proj_out")

        if memory_attention_pattern.match(key):
        ...
        if output_mask_decoder_mlps_pattern.match(key):
            layer_nb = int(output_mask_decoder_mlps_pattern.match(key).group(2))
            if layer_nb == 0:
        ...
        if output_mask_decoder_score_head_pattern.match(key):
            layer_nb = int(output_mask_decoder_score_head_pattern.match(key).group(1))
            if layer_nb == 0:
                key = key.replace("layers.0", "proj_in")
            elif layer_nb == 1:
        ...
        if output_hypernetworks_mlps_pattern.match(key):
            layer_nb = int(output_hypernetworks_mlps_pattern.match(key).group(2))
            if layer_nb == 0:
                key = key.replace("layers.0", "proj_in")
            elif layer_nb == 1:
                key = key.replace("layers.1", "layers.0")
        ...
        if output_vision_encoder_neck_pattern.match(key):
            key = key.replace(".conv.", ".")

        # memory_encoder.o_proj.weight -> memory_encoder.projection.weight
        if output_memory_encoder_projection_pattern.match(key):
            key = key.replace(".o_proj.", ".projection.")

        if output_object_pointer_proj_pattern.match(key):
            layer_nb = int(output_object_pointer_proj_pattern.match(key).group(1))
            if layer_nb == 0:
                key = key.replace("layers.0", "proj_in")
            elif layer_nb == 1:
                key = key.replace("layers.1", "layers.0")
        ...
        if output_memory_encoder_mask_downsampler_pattern.match(key):
            layer_nb = int(output_memory_encoder_mask_downsampler_pattern.match(key).group(1))
            if layer_nb == 12:

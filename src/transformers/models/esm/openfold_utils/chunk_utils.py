...
def _get_minimal_slice_set(
    ...
    # Reduce closure allocations by storing range slices and slice templates for reuse
    new_slice = slice

    def upper() -> tuple[tuple[slice, ...], ...]:
        sdi = start[divergence_idx]
        post_s = _get_minimal_slice_set(
            start[divergence_idx + 1 :],
            [d - 1 for d in dims[divergence_idx + 1 :]],
            dims[divergence_idx + 1 :],
            start_edges=start_edges[divergence_idx + 1 :],
            end_edges=[True] * (len(end_edges) - (divergence_idx + 1)),
        )

        p = path + (new_slice(sdi, sdi + 1),)
        return tuple(p + s for s in post_s)

    def lower() -> tuple[tuple[slice, ...], ...]:
        edi = end[divergence_idx]
        post_s = _get_minimal_slice_set(
            [0] * (len(start) - (divergence_idx + 1)),
            end[divergence_idx + 1 :],
            dims[divergence_idx + 1 :],
            start_edges=[True] * (len(start_edges) - (divergence_idx + 1)),
            end_edges=end_edges[divergence_idx + 1 :],
        )
        p = path + (new_slice(edi, edi + 1),)
        return tuple(p + s for s in post_s)

    se_is_edge = start_edges[divergence_idx]
    ee_is_edge = end_edges[divergence_idx]

    sdi = start[divergence_idx]
    edi = end[divergence_idx]
    # If both start and end are at the edges of the subtree rooted at
    # divergence_idx, we can just select the whole subtree at once
    if se_is_edge and ee_is_edge:
        slices.append(path + (slice(sdi, edi + 1),))
    # If just start is at the edge, we can grab almost all of the subtree,
    # treating only the ragged bottom edge as an edge case
    elif se_is_edge:
        slices.append(path + (slice(sdi, edi),))
        slices.extend(lower())
    # Analogous to the previous case, but the top is ragged this time
    elif ee_is_edge:
        slices.extend(upper())
        slices.append(path + (slice(sdi + 1, edi + 1),))
    ...
    # in one big chunk
        ...
        slices.extend(upper())
        middle_ground = edi - sdi
        if middle_ground > 1:
            slices.append(path + (slice(sdi + 1, edi),))
        slices.extend(lower())
...
def _chunk_slice(t: torch.Tensor, flat_start: int, flat_end: int, no_batch_dims: int) -> torch.Tensor:
    ...
    # Optimize slice application: fuse view/reshape operations in a single allocation
    # and preallocate storage list
    n_slices = len(slices)
    if n_slices == 1:
        # Avoid cat when only one slice
        res = t[slices[0]].view((-1,) + t.shape[no_batch_dims:])
        return res

    shape = (-1,) + t.shape[no_batch_dims:]
    out_list = [None] * n_slices
    for i, s in enumerate(slices):
        out_list[i] = t[s].view(shape)

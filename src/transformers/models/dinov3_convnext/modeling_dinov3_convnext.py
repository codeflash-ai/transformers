...
def drop_path(input: torch.Tensor, drop_prob: float = 0.0, training: bool = False) -> torch.Tensor:
    ...
    # Minor optimization: torch.empty() then inplace .uniform_ rather than allocating intermediates with rand()
    # This can improve performance by not allocating unnecessary objects if input is large
    random_tensor = torch.empty(shape, dtype=input.dtype, device=input.device).uniform_(0, 1)
    random_tensor.add_(keep_prob)
    random_tensor.floor_()  # binarize

    # Inplace multiplication reduces intermediate memory usage
    # input.div(keep_prob) is safe, as keep_prob > 0 always when drop_prob > 0
    output = input.div(keep_prob)

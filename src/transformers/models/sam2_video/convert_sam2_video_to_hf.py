...
def replace_keys(state_dict, config):
    model_state_dict = {}
    # Precompile all regex patterns for efficiency
    output_hypernetworks_mlps_regex = re.compile(r".*.output_hypernetworks_mlps.(\d+).layers.(\d+).*")
    output_mask_decoder_mlps_regex = re.compile(r"mask_decoder.transformer.layers.(\d+).mlp.layers.(\d+).*")
    output_mask_decoder_score_head_regex = re.compile(r"mask_decoder.pred_obj_score_head.layers.(\d+).*")
    output_vision_encoder_mlps_regex = re.compile(r"vision_encoder.backbone.blocks.(\d+).mlp.layers.(\d+).*")
    output_vision_encoder_neck_regex = re.compile(r"vision_encoder.neck.convs.(\d+).conv")
    output_memory_encoder_projection_regex = re.compile(r"memory_encoder.o_proj.*")
    output_object_pointer_proj_regex = re.compile(r"object_pointer_proj.layers.(\d+).*")
    output_memory_encoder_mask_downsampler_regex = re.compile(r"memory_encoder.mask_downsampler.encoder.(\d+).*")

    # Build sorted list of KEYS_TO_MODIFY_MAPPING keys by length descending for longest match preference
    # (avoid partials being matched before longer, more specific keys)
    keys_to_modify_sorted = sorted(KEYS_TO_MODIFY_MAPPING.keys(), key=len, reverse=True)

    # Iterate & transform keys
    for raw_key, value in state_dict.items():
        key = raw_key

        # Apply KEYS_TO_MODIFY_MAPPING replacements
        for key_to_modify in keys_to_modify_sorted:
            if key_to_modify in key:
                key = key.replace(key_to_modify, KEYS_TO_MODIFY_MAPPING[key_to_modify])
        # Pre-compile regex usage
        match = output_vision_encoder_mlps_regex.match(key)
        if match:
            ...
            match = output_mask_decoder_mlps_regex.match(key)
            if match:
                layer_nb = int(match.group(2))
                if layer_nb == 0:
                ...
                match = output_mask_decoder_score_head_regex.match(key)
                if match:
                    layer_nb = int(match.group(1))
                    if layer_nb == 0:
                        key = key.replace("layers.0", "proj_in")
                    elif layer_nb == 1:
                    ...
                    match = output_hypernetworks_mlps_regex.match(key)
                    if match:
                        layer_nb = int(match.group(2))
                        if layer_nb == 0:
                            key = key.replace("layers.0", "proj_in")
                        ...
                        if output_vision_encoder_neck_regex.match(key):
                            key = key.replace(".conv.", ".")
                        elif output_memory_encoder_projection_regex.match(key):
                            key = key.replace(".o_proj.", ".projection.")
                        else:
                            match = output_object_pointer_proj_regex.match(key)
                            if match:
                                layer_nb = int(match.group(1))
                                if layer_nb == 0:
                                    key = key.replace("layers.0", "proj_in")
                                ...
                                match = output_memory_encoder_mask_downsampler_regex.match(key)
                                if match:
                                    layer_nb = int(match.group(1))
                                    if layer_nb == 12:
    ...
    # Efficiently concatenate tensors using generator (avoid building unnecessary list in memory)
    weights_to_concat = (model_state_dict.pop(f"prompt_encoder.point_embed.{i}.weight") for i in range(4))
    model_state_dict["prompt_encoder.point_embed.weight"] = torch.cat(list(weights_to_concat), dim=0)

...
def get_image_size(image: np.ndarray, channel_dim: Optional[ChannelDimension] = None) -> tuple[int, int]:
    ...
    if channel_dim is None:
        # Attempt lightweight fast-path inference for common shapes
        shape = image.shape
        ndim = image.ndim
        # Avoid import and call to infer_channel_dimension_format if the shape is unambiguous and standard
        if ndim == 3:
            # Typical choices: (C, H, W) or (H, W, C)
            if shape[0] in (1, 3):
                ...
                # Fallback: call original inference
            ...
            # Other shapes: fallback to expensive inference
            from transformers.image_utils import infer_channel_dimension_format

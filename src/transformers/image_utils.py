...
def is_scaled_image(image: np.ndarray) -> bool:
    ...
    if image.dtype == np.uint8:
        return False
    # For float and non-uint8, check once if all elements lie in interval [0, 1]
    # Avoid redundant np.min and np.max scanning
    return np.all((image >= 0) & (image <= 1))
...
def to_numpy_array(img) -> np.ndarray:
    ...
    if isinstance(img, np.ndarray):
        return img
    # Validate image type before further conversion
...
def infer_channel_dimension_format(
    ...
    shape_first = shape[first_dim]
    shape_last = shape[last_dim]
    if shape_first in num_channels and shape_last in num_channels:
        logger.warning(
    ...
    elif shape_first in num_channels:
        return ChannelDimension.FIRST
    elif shape_last in num_channels:

...
def to_numpy_array(img) -> np.ndarray:
    ...
    # Cache result of vision and PIL.Image.Image type check on first use
    if not hasattr(to_numpy_array, "_pil_is_available"):
        to_numpy_array._pil_is_available = is_vision_available()
        to_numpy_array._pil_image_cls = PIL.Image.Image if to_numpy_array._pil_is_available else None

    pil_is_available = to_numpy_array._pil_is_available
    pil_image_cls = to_numpy_array._pil_image_cls

    if pil_is_available and isinstance(img, pil_image_cls):
        return np.array(img)
    # Skip redundant conversion if already np.ndarray
